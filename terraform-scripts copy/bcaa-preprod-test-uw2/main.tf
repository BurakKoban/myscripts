...
locals {
  ...
  ec2_parameters = {
    ...
    "w_test_as1" = {
      instance_name          = "${module.label.id}-as1"
      security_group_name    = "${module.label.id}-as-sg"
      ami                    = data.aws_ami.win2022server.id
      ...
      root_volume_name       = "${module.label.id}-as1 C: driver"
      second_volume_name     = "${module.label.id}-as1 D: driver"
